...
int main() {
    ...
    vector<int> resource = {15, 8};
    NN rm(resource);

    TT ZZ(3, {2, 4, 6}, {3, 6, 9});

    auto p1 = make_shared<CC>(1, AA::SYSTEM, 10, vector<int>{5, 2});
    auto p2 = make_shared<CC>(2, AA::INTERACTIVE, 10, vector<int>{3, 1});
    auto p3 = make_shared<CC>(3, AA::BATCH, 10, vector<int>{4, 2});
    ...
    while (timeStep < maxTime) {
        ...
        cout << "Running CC " << current->DD << " (" << typeStr
             << ", Level " << current->HH << ") for " << quantum << " time units\n";

        current->GG -= quantum;
        if (current->GG < 0) current->GG = 0;
