...
main(){
        ...
        if(l==0){
            ...
            for(int i=0;i<q[l].length;i++){
                ...
                while(rr_timer>0 && q[l].p[i].burst_time!=0 && timer!=10){
                    if(q[l].p[i].start_time == -1){
                        q[l].p[i].start_time = global_time;
                    }
    ...
    for(int i=0;i<3;i++){
            cout<<"Queue "<<i+1<<"\n";
        for(int j=0;j<q[i].length;j++){
            float tat = q[i].p[j].end_time - 0;  // Completion time - Arrival time (0)
            float wt = tat - q[i].p[j].tt_time;   // TAT - Burst time
            cout<<"Process P"<<j+1<<"\t"<<tat<<"\t\t    "<<wt<<"\n";
            sum_tt+=tat;
            sum_wt+=wt;
        }
    }
